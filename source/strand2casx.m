function [ point_coordinates, arc_connectivity, arc_diameters, point_radii ] = strand2casx( strand_subscripts, microns_per_voxel, lumen_radius_in_microns_range )
...
[ unique_terminal_points, idcs, strands2unique_vertices ] = unique( strand_endpoint_subscripts( :, 1 : 3 ), 'rows' );

unique_terminal_points = strand_endpoint_subscripts( idcs, : );
...
for strand_index = strand_index_range
    ...
    for strand_point_index = 1 : number_of_interior_points_in_strands( strand_index )
        ...
        point_coordinates( number_of_added_points, : ) = strand_subscripts{ strand_index }( strand_point_index + 1, : );
...
%  convert radius index to microns
  arc_radii = exp( interp1( log( lumen_radius_in_microns_range ), arc_radius_indices       ));
point_radii = exp( interp1( log( lumen_radius_in_microns_range ), point_coordinates( :, 4 )));
...
% convert voxel address to microns:
